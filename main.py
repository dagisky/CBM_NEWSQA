...
def train(model, x, mask, p1, p2, seqlens, mloss, optim, args):
    ...
    print(seqlens.size())
    print("---------------------") 
    pred, std = model(torch.transpose(x[0].to(args.device), -2,-1), mask, seqlens) 
...
def main():
    ...
    all_end_pos = torch.tensor([f.end_position for f in features], dtype=torch.long,device=device)
    all_segment_ids = torch.tensor([f.segment_ids for f in features], dtype=torch.long, device=device)
    all_example_index = torch.arange(all_input_ids.size(0), dtype=torch.long, device=device)
